...
-- ---------- 5. AUTO-PROFIL SAAT USER DIBUAT ----------
-- Setiap kali user baru dibuat di Authentication, profilnya otomatis dibuat
-- (peran awal 'petugas'). Jadi Anda TIDAK perlu menyalin UUID manual.
create or replace function public.handle_new_user()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  insert into public.profiles (id, nama, peran)
  values (new.id, coalesce(new.raw_user_meta_data->>'nama', split_part(new.email,'@',1)), 'petugas')
  on conflict (id) do nothing;
  return new;
end; $$;
...
-- =====================================================================
-- 6. CARA MEMBUAT AKUN LOGIN (lakukan SETELAH menjalankan skema di atas)
-- ---------------------------------------------------------------------
-- LANGKAH 1: Buka Supabase > Authentication > Users > "Add user".
--            Isi Email & Password, lalu CENTANG "Auto Confirm User".
--            (mis. email: admin@dlhkepahiang.go.id  password: pilih sendiri)
--
-- LANGKAH 2: Profil otomatis terbuat sebagai 'petugas'. Untuk menjadikannya
--            ADMIN, jalankan perintah ini di SQL Editor (ganti email sesuai):
--
...
--
--    -- Untuk peran lain, ganti 'admin' dengan 'petugas' atau 'pimpinan'.
--
-- LANGKAH 3: Buka aplikasi, menu "Petugas / Admin", login dengan email &
--            password tersebut. Selesai.
-- =====================================================================

-- (Opsional) Lihat daftar akun & perannya:
